...
export class InventoryComponent implements OnInit, OnDestroy {
  ...
  loadInventory() {
    this.loading = true;
    this.inventory = []; // Clear previous data
    ...
    this.inventoryService.searchInventory(this.searchTerm, this.searchType, this.searchPage, this.pageSize)
      .subscribe({
        next: (data) => {
          ...
          if (data.length === 0) {
            this.toastService.showInfo('No inventory items found matching your search criteria.');
          } else {
            this.toastService.showSuccess(`Found ${data.length} inventory items.`);
          }
        },
        error: (error) => {
